...
def test_user_prompt_flat_position():
    inp = OracleInput(date_str="2026-04-20", current_price=700.0, recent_closes=[700.0], position_qty=0)
    prompt = _build_user_prompt(inp)
    assert "flat" in prompt


def test_parse_json_plain():
    text = '{"action": "BUY", "confidence": "HIGH", "regime_assessment": "BULL", "key_drivers": ["a"], "reasoning": "x"}'
...
def test_query_claude_parses_response(mock_anthropic_cls, sample_input):
    ...
    text_block.text = (
        ...
        '"key_drivers": ["momentum", "breadth"], "reasoning": "uptrend intact"}'
    )
    response = MagicMock()
...
def test_query_claude_filters_tool_use_blocks(mock_anthropic_cls, sample_input):
    ...
    text_block = MagicMock()
    text_block.type = "text"
    text_block.text = '{"action": "HOLD", "confidence": "LOW", "regime_assessment": "CHOPPY"}'
    response = MagicMock()
...
def test_query_openai_parses_response(mock_openai_cls, sample_input):
    ...
    response.output_text = (
        '{"action": "SELL", "confidence": "MEDIUM", "regime_assessment": "BEAR", '
        '"key_drivers": ["breadth"], "reasoning": "weakening"}'
    )
...
def test_query_all_returns_both_even_on_failure(mock_claude, mock_openai, sample_input):
    ...
    mock_claude.return_value = OracleResponse(
        provider="claude", model="m", action="BUY", confidence="HIGH",
        regime_assessment="BULL", key_drivers=[], reasoning="", raw_response="",
    )
